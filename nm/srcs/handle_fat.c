...
int		nm_inside_fat(void *ptr, size_t size, char *file, char *name)
{
	void	(*f)(t_env*);
	t_env	*env;
	int		ret;
...
void	fat_print_all(t_env *env, struct fat_arch *a, uint32_t n)
{
		...
		sizeof(struct fat_arch), env) && env->error == 0)
	{
		if (check_addr(env->ptr + a->offset, a->size, env))
			env->error = nm_inside_fat(env->ptr + a->offset, a->size, env->file,
				arch_get_name(a->cputype, a->cpusubtype));
		a++;
		i++;
	}
}
...
void	handle_fat(t_env *env)
{
	...
	if (env->error == 0 && safe)
		env->error = nm(env->ptr + safe->offset, safe->size, env->file);
